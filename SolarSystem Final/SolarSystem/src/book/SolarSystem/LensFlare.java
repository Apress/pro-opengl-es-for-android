...
import book.SolarSystem.SolarSystemView.SolarSystem.CGPoint;
...
public class LensFlare 
{
	...
	public void createFlares(GL10 gl, Context context)
	{
	    ...
	    resid = book.SolarSystem.R.drawable.hexagonblur;
	    m_Flares[0] = myFlares[0].init(gl, context, resid, .5f, .05f-ff, 1.0f, .73f, .30f, .4f);

	    resid = book.SolarSystem.R.drawable.glow;
	    ...
	    resid = book.SolarSystem.R.drawable.halo;
	    ...
	    resid = book.SolarSystem.R.drawable.hexagon;
	    ...
	    resid = book.SolarSystem.R.drawable.flarefuzzy;
	...
	public void execute(GL10 gl,CGSize size, CGPoint source, float scale, float alpha)
	{
	    ...
	    for (i = 0; i < 24; i++) 
	    {
	    	...
	    	myFlares[i].renderFlareAt(gl, m_Flares[i], offsetFromCenterX, offsetFromCenterY, size, this.context, scale, alpha);    
	    }
